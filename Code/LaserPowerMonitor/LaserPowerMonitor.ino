...
void setup() {
  ...
  delay(1000); 
  Serial.print("Timer setup Complete");
  delay(1000);
  //Serial.println("");
  ...
  for (int i=0; i <= 100; i++) {
    ...
    if ((userReset == LOW) && !lastLaserOn && userMillis > 0) {
      ...
      if (millis() > (lastWriteToEEPROMMillis+10000)) {
        ...
        Serial.print("User hit reset & Wrote to EEPROM");
        delay(200);
      ...
      lcd.setCursor(0,1);
      lcd.print(buffer2);
      Serial.println(timeChar+laserTime.seconds);
      delay(200);
      Serial.println(writeChar+laserTime.EEPROMwriteCount);
      delay(3600);
